import Link from "next/link";
import { notFound } from "next/navigation";
import { AdminShell } from "@/components/admin-shell";
import { updateFinanceEntryAction } from "@/app/finance/[id]/edit/actions";
import { getFinanceEntryById } from "@/lib/finance-store";
import { getOrders } from "@/lib/order-store";

const fieldClassName =
  "mt-2 w-full rounded-2xl border border-[color:var(--line)] bg-white px-4 py-3 text-sm outline-none transition placeholder:text-[#93a09d] focus:border-[color:var(--accent)]";

function toDatetimeLocalValue(value: string) {
  if (!value) {
    return "";
  }

  if (value.includes("今天") || value.includes("昨天")) {
    return "";
  }

  const normalized = value.replace(" ", "T");
  return normalized.length === 16 ? normalized : normalized.slice(0, 16);
}

function toNumericAmount(value: string) {
  return String(Number(value.replace(/[^\d.-]/g, "")) || 0);
}

export default async function EditFinancePage({
  params,
  searchParams,
}: {
  params: Promise<{ id: string }>;
  searchParams: Promise<{ error?: string }>;
}) {
  const { id } = await params;
  const query = await searchParams;
  const [entry, orders] = await Promise.all([getFinanceEntryById(id), getOrders()]);

  if (!entry) {
    notFound();
  }

  return (
    <AdminShell
      activeHref="/finance"
      title="编辑流水"
      description="修改流水类型、金额、关联订单和备注，并自动刷新订单实收汇总。"
      aside={
        <>
          <p className="text-sm font-semibold">编辑说明</p>
          <p className="mt-2 text-sm leading-6 muted">
            如果这笔流水关联了订单，修改后对应订单的已收和待收会自动重新计算。
          </p>
        </>
      }
    >
      <div className="rounded-[2rem] bg-[linear-gradient(135deg,#21443f_0%,#49756b_42%,#f0a07a_100%)] px-6 py-7 text-white shadow-2xl shadow-orange-100/70">
        <p className="text-sm uppercase tracking-[0.24em] text-white/74">Edit Finance</p>
        <h2 className="mt-3 text-2xl font-semibold sm:text-3xl">{entry.title}</h2>
        <p className="mt-3 max-w-2xl text-sm leading-7 text-white/82">
          调整流水类型、金额、发生时间、关联订单和往来方信息。
        </p>
      </div>

      <section className="soft-card rounded-[1.75rem] p-5">
        {query.error === "missing" ? (
          <div className="mb-5 rounded-2xl border border-[#f0c8b2] bg-[#fff4ee] px-4 py-3 text-sm text-[#a3512d]">
            请先补全流水类型、摘要和金额这几个必填项。
          </div>
        ) : null}

        <form action={updateFinanceEntryAction}>
          <input type="hidden" name="entryId" value={entry.id} />

          <div className="grid gap-5 lg:grid-cols-2">
            <label className="text-sm font-medium">
              流水类型
              <select name="type" required className={fieldClassName} defaultValue={entry.type}>
                <option>收款</option>
                <option>退款</option>
                <option>支出</option>
              </select>
            </label>
            <label className="text-sm font-medium">
              流水摘要
              <input
                name="title"
                required
                defaultValue={entry.title}
                className={fieldClassName}
              />
            </label>
            <label className="text-sm font-medium">
              金额
              <input
                name="amount"
                required
                defaultValue={toNumericAmount(entry.amount)}
                className={fieldClassName}
              />
            </label>
            <label className="text-sm font-medium">
              发生时间
              <input
                name="happenedAt"
                type="datetime-local"
                defaultValue={toDatetimeLocalValue(entry.time)}
                className={fieldClassName}
              />
            </label>
            <label className="text-sm font-medium">
              关联订单
              <select name="orderId" className={fieldClassName} defaultValue={entry.orderId ?? ""}>
                <option value="">暂不关联订单</option>
                {orders.map((order) => (
                  <option key={order.id} value={order.id}>
                    {order.customer} · {order.id}
                  </option>
                ))}
              </select>
            </label>
            <label className="text-sm font-medium">
              流水分类
              <select name="category" className={fieldClassName} defaultValue={entry.category ?? ""}>
                <option value="">请选择分类</option>
                <option>订单定金</option>
                <option>订单尾款</option>
                <option>订单全款</option>
                <option>加选增购</option>
                <option>摄影师成本</option>
                <option>后期修图</option>
                <option>交通支出</option>
                <option>道具物料</option>
                <option>售后退款</option>
              </select>
            </label>
            <label className="text-sm font-medium">
              往来方
              <input
                name="counterparty"
                defaultValue={entry.counterparty ?? ""}
                className={fieldClassName}
              />
            </label>
          </div>

          <label className="mt-5 block text-sm font-medium">
            备注
            <textarea
              name="notes"
              defaultValue={entry.notes ?? ""}
              className={`${fieldClassName} min-h-32 resize-y`}
            />
          </label>

          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <button className="rounded-full bg-[color:var(--accent)] px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-orange-200/70 transition hover:brightness-105">
              保存修改
            </button>
            <Link
              href="/finance"
              className="rounded-full border border-[color:var(--line)] px-5 py-3 text-center text-sm font-semibold hover:bg-white"
            >
              返回流水列表
            </Link>
          </div>
        </form>
      </section>
    </AdminShell>
  );
}
